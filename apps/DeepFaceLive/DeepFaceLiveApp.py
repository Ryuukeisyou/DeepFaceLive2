...
from .ui.QLivePortraitPostprocess import QLivePortraitPostprocess
from .ui.QFaceDetector import QFaceDetector
from .ui.QFaceCropper import QFaceCropper
...
from .ui.widgets.QBCCropViewer import QBCCropViewer
...
class QLiveSwap(qtx.QXWidget):
    def __init__(self, userdata_path : Path,
        ...
        face_cropper_bc_out   = backend.BackendConnection()
        ...
        live_portrait_bc_out  = backend.BackendConnection()
        ...
        face_cropper      = self.face_cropper      = backend.FaceCropper             (weak_heap=backend_weak_heap, reemit_frame_signal=reemit_frame_signal, bc_in=face_detector_bc_out, bc_out=face_cropper_bc_out, backend_db=backend_db)
        ...
        lp_animator       = self.lp_animator       = backend.FasterLivePortraitAnimator(weak_heap=backend_weak_heap, reemit_frame_signal=reemit_frame_signal, bc_in=face_cropper_bc_out, bc_out=face_merger_bc_out, animatables_path=animatables_path, backend_db=backend_db )
        lp_postprocess    = self.lp_postprocess    = backend.LivePortraitPostprocess (weak_heap=backend_weak_heap, reemit_frame_signal=reemit_frame_signal, bc_in=live_portrait_bc_out, bc_out=face_merger_bc_out, backend_db=backend_db )
        ...
        self.all_backends : List[backend.BackendHost] = [file_source, camera_source, face_detector, face_cropper, face_marker, face_aligner, face_animator, lp_animator, lp_postprocess, face_swap_insight, face_swap_dfm, frame_adjuster, face_merger, stream_output]
        ...
        self.q_face_cropper      = QFaceCropper(self.face_cropper)
        ...
        self.q_lp_animator       = QLivePortraitAnimator(self.lp_animator, animatables_path=animatables_path)
        self.q_lp_postprocess    = QLivePortraitPostprocess(self.lp_postprocess)
        ...
        self.q_ds_cr_viewer           = QBCCropViewer(backend_weak_heap, face_cropper_bc_out, preview_width=256)
        ...
        q_nodes = qtx.QXWidgetHBox([    qtx.QXWidgetVBox([self.q_file_source, self.q_camera_source], spacing=5, fixed_width=256),
                                        qtx.QXWidgetVBox([self.q_face_detector, self.q_face_cropper], spacing=5, fixed_width=256),
                                        qtx.QXWidgetVBox([self.q_face_marker, self.q_face_aligner], spacing=5, fixed_width=256),
                                        qtx.QXWidgetVBox([self.q_face_animator, self.q_lp_animator, self.q_lp_postprocess, self.q_face_swap_insight, self.q_face_swap_dfm], spacing=5, fixed_width=256),
        ...
        q_view_nodes = qtx.QXWidgetHBox([   (qtx.QXWidgetVBox([self.q_ds_frame_viewer], fixed_width=256), qtx.AlignTop),
                                            (qtx.QXWidgetVBox([self.q_ds_cr_viewer], fixed_width=256), qtx.AlignTop),
